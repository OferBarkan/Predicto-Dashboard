...
two_days_before = datetime.strptime(date_str, "%Y-%m-%d") - timedelta(days=2)
...
roas_2prev = roas_df[roas_df["Date"] == two_days_before_str][[
    "Ad Name", "Custom Channel ID", "Search Style ID", "ROAS"
...
for col in ["Ad Name", "Custom Channel ID", "Search Style ID"]:
    ...
    roas_2prev[col] = roas_2prev[col].astype(str).str.strip()

df = df.merge(roas_prev, on=["Ad Name", "Custom Channel ID", "Search Style ID"], how="left")
df = df.merge(roas_2prev, on=["Ad Name", "Custom Channel ID", "Search Style ID"], how="left")
...
if selected_style != "All":
    ...
    roas_color = "#B31B1B" if style_roas < 0.7 else "#FDC1C5" if style_roas < 0.95 else "#FBEEAC" if style_roas < 1.10 else "#93C572" if style_roas < 1.4 else "#019529"
